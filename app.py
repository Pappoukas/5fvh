...
from utils.website_analytics import load_website_data
...
# ------------------------------------------------- WEBSITE ANALYTICS ----
st.subheader("🔍 Απόδοση Ιστοσελίδας (Google Search)")
website_data = load_website_data()

if not website_data:
    st.info(
        "Δεν βρέθηκαν δεδομένα ιστοσελίδας. Πρόσθεσε exports Google Search Console "
        "(.xlsx, 'Performance on Search') στον φάκελο `data/website/` για να ενεργοποιηθεί "
        "αυτή η ενότητα."
    )
else:
    daily_web = website_data.get("daily")
    if daily_web is not None and len(daily_web):
        st.caption(
            "Πηγή: Google Search Console, chaniabookfestival.gr — "
            f"{daily_web['date'].min().strftime('%d/%m/%Y')} έως "
            f"{daily_web['date'].max().strftime('%d/%m/%Y')}. Μόνο οργανική αναζήτηση Google "
            "(όχι direct traffic, social referrals ή διαφημίσεις)."
        )

        wk1, wk2, wk3, wk4 = st.columns(4)
        wk1.metric("Σύνολο κλικ (οργανικά)", f"{int(daily_web['clicks'].sum()):,}")
        wk2.metric("Σύνολο εμφανίσεων", f"{int(daily_web['impressions'].sum()):,}")
        total_impr = daily_web["impressions"].sum()
        avg_ctr = daily_web["clicks"].sum() / total_impr if total_impr else 0
        wk3.metric("Μέσο CTR", f"{avg_ctr:.1%}")
        wk4.metric("Μέση θέση Google", f"{daily_web['position'].mean():.1f}")

        fig_web = make_subplots(specs=[[{"secondary_y": True}]])
        fig_web.add_trace(go.Scatter(
            x=daily_web["date"], y=daily_web["clicks"], name="Κλικ",
            mode="lines", line=dict(color="#1B4A5A", width=2),
        ), secondary_y=False)
        fig_web.add_trace(go.Scatter(
            x=daily_web["date"], y=daily_web["impressions"], name="Εμφανίσεις",
            mode="lines", line=dict(color="#C9953A", width=2, dash="dot"),
        ), secondary_y=True)
        for yr in sorted(daily_web["year"].dropna().unique()):
            if yr in FESTIVAL_DATES:
                start, end = FESTIVAL_DATES[yr]
                fig_web.add_vrect(
                    x0=start, x1=end, fillcolor="orange", opacity=0.15, line_width=0,
                    annotation_text=EDITION_LABELS.get(yr, str(yr)), annotation_position="top left",
                )
        fig_web.update_yaxes(title_text="Κλικ", secondary_y=False)
        fig_web.update_yaxes(title_text="Εμφανίσεις", secondary_y=True, showgrid=False)
        fig_web.update_layout(height=450, legend=dict(orientation="h", yanchor="bottom", y=1.02, x=0))
        st.plotly_chart(fig_web, width='stretch')

        st.markdown("#### 🔗 Σύγκριση με προβολές social media (ίδια περίοδο)")
        web_years = sorted(daily_web["year"].dropna().unique())
        overlap = owned[owned["year"].isin(web_years)]
        if len(overlap):
            social_daily = overlap.groupby("date")["views"].sum().reset_index()
            social_daily["date"] = pd.to_datetime(social_daily["date"])
            merged = pd.merge(daily_web[["date", "clicks"]], social_daily, on="date", how="outer").sort_values("date")
            fig_cross = make_subplots(specs=[[{"secondary_y": True}]])
            fig_cross.add_trace(go.Scatter(
                x=merged["date"], y=merged["clicks"], name="Κλικ ιστοσελίδας",
                mode="lines", line=dict(color="#1B4A5A", width=2),
            ), secondary_y=False)
            fig_cross.add_trace(go.Bar(
                x=merged["date"], y=merged["views"], name="Προβολές social",
                marker=dict(color="#C9953A", opacity=0.35),
            ), secondary_y=True)
            fig_cross.update_yaxes(title_text="Κλικ ιστοσελίδας", secondary_y=False)
            fig_cross.update_yaxes(title_text="Προβολές social", secondary_y=True, showgrid=False)
            fig_cross.update_layout(height=450, legend=dict(orientation="h", yanchor="bottom", y=1.02, x=0))
            st.plotly_chart(fig_cross, width='stretch')
            valid = merged.dropna(subset=["clicks", "views"])
            corr = valid["clicks"].corr(valid["views"]) if len(valid) >= 5 else float("nan")
            st.caption(
                f"Συσχέτιση κλικ ιστοσελίδας – προβολών social ίδιας ημέρας: r ≈ {corr:.2f}."
                if pd.notna(corr) else
                "Ανεπαρκή επικαλυπτόμενα δεδομένα (φιλτραρισμένα κατά κανάλι/έτος) για συσχέτιση."
            )
        else:
            st.info("Δεν υπάρχουν δεδομένα social media για την περίοδο που καλύπτει το website export, με τα τρέχοντα φίλτρα.")

    st.markdown("---")
    qcol1, qcol2 = st.columns(2)
    with qcol1:
        if "queries" in website_data:
            st.markdown("**🔎 Κορυφαίες αναζητήσεις Google**")
            top_q = website_data["queries"].nlargest(15, "clicks")[["query", "clicks", "impressions", "position"]].copy()
            top_q.columns = ["Αναζήτηση", "Κλικ", "Εμφανίσεις", "Μέση θέση"]
            st.dataframe(top_q, width='stretch', hide_index=True)
    with qcol2:
        if "pages" in website_data:
            st.markdown("**📄 Κορυφαίες σελίδες**")
            top_p = website_data["pages"].nlargest(15, "clicks")[["page", "clicks", "impressions", "position"]].copy()
            top_p["page"] = top_p["page"].str.replace("https://www.chaniabookfestival.gr", "", regex=False)
            top_p.columns = ["Σελίδα", "Κλικ", "Εμφανίσεις", "Μέση θέση"]
            st.dataframe(top_p, width='stretch', hide_index=True)

    dcol1, dcol2 = st.columns(2)
    with dcol1:
        if "devices" in website_data:
            st.markdown("**📱 Συσκευές**")
            dev = website_data["devices"].groupby("device").agg(clicks=("clicks", "sum")).reset_index()
            fig_dev = px.pie(dev, names="device", values="clicks")
            st.plotly_chart(fig_dev, width='stretch')
    with dcol2:
        if "countries" in website_data:
            st.markdown("**🌍 Χώρες (top 10 εκτός Ελλάδας)**")
            ctry = website_data["countries"].groupby("country").agg(clicks=("clicks", "sum")).reset_index()
            ctry_intl = ctry[ctry["country"] != "Greece"].nlargest(10, "clicks")
            fig_ctry = px.bar(ctry_intl, x="country", y="clicks", labels={"country": "", "clicks": "Κλικ"})
            st.plotly_chart(fig_ctry, width='stretch')
            gr_clicks = int(ctry.loc[ctry["country"] == "Greece", "clicks"].sum())
            total_clicks = int(ctry["clicks"].sum())
            st.caption(f"Η Ελλάδα αντιπροσωπεύει {gr_clicks:,} από {total_clicks:,} κλικ ({gr_clicks/total_clicks:.0%}) — εξαιρέθηκε από το γράφημα για να αναδειχθεί το διεθνές κοινό.")
